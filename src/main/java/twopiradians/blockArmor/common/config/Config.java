...
public class Config 
{
	...
	public static void syncConfig() 
	{
		...
		for (String modid : ArmorSet.armorSetMods.keySet()) {
			...
			if (mod != null) {
				...
				for (ArmorSet set : ArmorSet.armorSetMods.get(modid)) {
					...
					Property prop = Config.config.get(ARMOR_SETS_CATEGORY+"."+mod.getName(), name+" Armor Set", true,
		...
		for (SetEffect effect : SetEffect.SET_EFFECTS) {
			...
			Property prop = Config.config.get(Config.SET_EFFECTS_CATEGORY, name, true, "Determines whether or not the "+name+" set effect can be used.");
			if (!prop.getBoolean())
