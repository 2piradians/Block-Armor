...
public class TileEntityMovingLightSource extends TileEntity implements ITickable
{
	...
	public void update() {
		...
		EntityPlayer player = worldObj.getClosestPlayer(getPos().getX()+0.5D, getPos().getY()+0.5D, getPos().getZ()+0.5D, 2.0D, false);
		if ((player == null || !ArmorSet.isWearingFullSet(player, null)) &&
				worldObj.getBlockState(getPos()).getBlock() instanceof BlockMovingLightSource) {
