...
public class SetEffectLucky extends SetEffect {
	...
	protected SetEffectLucky() {
		...
		this.attributeModifiers.add(new AttributeModifier(LUCK_UUID, 
				SharedMonsterAttributes.LUCK.getAttributeUnlocalizedName(), 3, 0));
	...
	public void addLooting(LootingLevelEvent event) {
		if (event.getDamageSource().getEntity() instanceof EntityPlayer && 
				event.getEntity().worldObj instanceof WorldServer &&
			...
			doParticlesAndSound((WorldServer) event.getEntity().worldObj, event.getEntity().getPosition(), 
