...
*//**
	 * Actually runs the command (for chat event), returns if message was a valid
	 * command (and chat should be hidden)
	 *//*
		...
		ItemStack stack = ((PlayerEntity) sender).getItemStackFromSlot(slot);
		ItemStack newStack = new ItemStack(set.getArmorForSlot(slot));
		ListNBT nbt = new ListNBT();
		nbt.setBoolean("devSpawned", true);
		newStack.setTag(nbt);
		if (stack == null || stack.getItem() instanceof ItemAir || stack.getItem() instanceof BlockArmorItem)
		((PlayerEntity) sender).setItemStackToSlot(slot, newStack);
		}
		sender.sendMessage(new TextComponentTranslation(TextFormatting.GREEN+"Spawned set for "+args[1].replace("_", " ")));
		}
		else
		sender.sendMessage(new TextComponentTranslation(TextFormatting.RED+"Invalid block"));
		return true;
		}
		...
		devColors.remove(((PlayerEntity) sender).getPersistentData());
		else
		devColors.put(((PlayerEntity) sender).getPersistentData(), color);
		BlockArmor.NETWORK.sendToAll(new SDevColorsPacket());
		}
		catch (Exception e) {
		sender.sendMessage(new TextComponentTranslation(TextFormatting.RED+"Color must be 3 floats (-1 to 1 for standard results)"));
		}
		return true;
		}
		}
		...
		if (sender instanceof PlayerEntity)
		return DEVS.contains(((PlayerEntity) sender).getPersistentData());
		return false;
		}
